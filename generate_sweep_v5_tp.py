...
# Parameter Ranges for V5 (Persistence & Smoothing)
distances = [18.0, 20.0, 22.0, 24.0] 
persistences = [2.0, 2.5, 3.0]
smoothings = [0.1, 0.2]
    ...
    for persist in persistences:
        for smooth in smoothings:
            config = {
                ...
                "H_TARGET_REF": 0.6,
                "HFOV_DEG": 85,
                "EMA_ALPHA": smooth,     # Variable
                "EMA_ALPHA_V": 0.1,      # Fixed
                "DIST_WARN_M": 35.0,     # Fixed
                "DIST_DANGER_M": dist,   # Variable
                "DANGER_PERSISTENCE_SEC": persist, # Variable
            ...
            job_name = f"sweep-v5-tp-exp-{exp_id:02d}"
            job_file = f"job_sweep_v5_tp_exp_{exp_id:02d}.json"
            ...
            # Inject Output Path
            job['taskGroups'][0]['taskSpec']['volumes'][3]['gcs']['remotePath'] = f"yolo-gcp/eagle/infer/output/sweep_v5_tp/{job_name}/"
            ...
            print(f"Generated {job_file}: Dist={dist}, Persist={persist}, Smooth={smooth}")
            exp_id += 1

# Generate Submission Script
with open("submit_sweep_v5_tp.sh", "w") as f:
    f.write("#!/bin/bash\n")
...
print(f"Generated {len(experiments)} TP experiments.")
