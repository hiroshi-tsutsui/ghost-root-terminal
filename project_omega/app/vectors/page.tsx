...
function VectorArrow({ start = [0, 0, 0], end, color = 'orange', label = '' }: { start?: [number, number, number], end: [number, number, number], color?: string, label?: string }) {
  ...
  return (
    <group position={startVec}>
      ...
      {label && (
        <Text
          position={endVec.clone().sub(startVec).add(new THREE.Vector3(0, 0.2, 0))}
          fontSize={0.3}
          color="black"
          anchorX="center"
          anchorY="middle"
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-80 p-4 bg-gray-50 border-r overflow-y-auto space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              <label className="text-sm">X <input type="number" value={v1[0]} onChange={(e) => updateV1(0, parseFloat(e.target.value))} className="w-full border p-1 rounded" /></label>
              <label className="text-sm">Y <input type="number" value={v1[1]} onChange={(e) => updateV1(1, parseFloat(e.target.value))} className="w-full border p-1 rounded" /></label>
              <label className="text-sm">Z <input type="number" value={v1[2]} onChange={(e) => updateV1(2, parseFloat(e.target.value))} className="w-full border p-1 rounded" /></label>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              <label className="text-sm">X <input type="number" value={v2[0]} onChange={(e) => updateV2(0, parseFloat(e.target.value))} className="w-full border p-1 rounded" /></label>
              <label className="text-sm">Y <input type="number" value={v2[1]} onChange={(e) => updateV2(1, parseFloat(e.target.value))} className="w-full border p-1 rounded" /></label>
              <label className="text-sm">Z <input type="number" value={v2[2]} onChange={(e) => updateV2(2, parseFloat(e.target.value))} className="w-full border p-1 rounded" /></label>
            </div>
          </div>
          ...
          <div className="p-4 bg-white rounded shadow-sm border space-y-2 text-sm">
            ...
            <p><strong>|A|:</strong> {vec1.length().toFixed(2)}</p>
            <p><strong>|B|:</strong> {vec2.length().toFixed(2)}</p>
        ...
        <div className="flex-1 bg-gray-900 relative">
          ...
          <div className="absolute top-4 left-4 text-white bg-black/50 p-2 rounded text-xs">
             ...
             <span className="text-green-400 font-bold ml-2">Y (Green)</span>
             <span className="text-blue-400 font-bold ml-2">Z (Blue)</span>
          </div>
        </div>
      </div>
    </div>
