...
# Focusing on higher distances based on V4 trend
...
for dist in distances:
    for persist in persistences:
        for smooth in smoothings:
            ...
            job_name = f"sweep-v5-exp-{exp_id:02d}"
            job_file = f"job_sweep_v5_exp_{exp_id:02d}.json"
            ...
            # Inject Output Path
            job['taskGroups'][0]['taskSpec']['volumes'][3]['gcs']['remotePath'] = f"yolo-gcp/eagle/infer/output/sweep_v5/{job_name}/"
...
with open("submit_sweep_v5.sh", "w") as f:
    f.write("#!/bin/bash\n")
